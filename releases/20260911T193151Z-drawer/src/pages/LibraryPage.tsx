...
export function LibraryDrawer({ open, onClose }: LibraryDrawerProps) {
  ...
  useEffect(() => {
    if (!open || packagesRequestedRef.current) return
    ...
    listServerPackages()
      ...
      .catch((reason) => {
        ...
        setError(reason instanceof Error ? reason.message : '无法读取服务器曲库')
      })
      .finally(() => {
  ...
  return (
    <>
      <button
        className="library-floating-pause"
        type="button"
        hidden={open || !playingKey}
        onClick={() => audioRef.current?.pause()}
        aria-label={`暂停 ${nowPlaying?.workName || '曲库试听'}`}
      >
        <span aria-hidden="true">Ⅱ</span>
        <span>暂停试听</span>
      </button>
      <div
        className={`library-drawer-backdrop${open ? ' open' : ''}`}
        aria-hidden="true"
        onMouseDown={onClose}
      />
      <aside
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="libraryDrawerTitle"
        aria-hidden={!open}
        inert={!open}
      >
        <header className="library-drawer-header">
          <div>
            ...
            <h1 id="libraryDrawerTitle">曲库预览</h1>
          </div>
          <button className="library-drawer-close" type="button" onClick={onClose} aria-label="关闭曲库预览">
            <span aria-hidden="true">×</span>
          </button>
        </header>

        <section className="library-drawer-toolbar" aria-label="曲库选择">
          <div className="field">

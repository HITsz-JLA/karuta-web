...
const PreviewCardTile = function PreviewCardTile({
  ...
  return (
    ...
    >
      ...
      <span className="muted small">{card.songCount} 首歌曲 · 点击查看</span>
    </button>
  )
}
...
export function PackagePreviewPage() {
  ...
  const [packageInfo, setPackageInfo] = useState<ServerPackage | null>(null)
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([getServerPackageCatalog(packageId), listServerPackages()]).then((results) => {
      ...
      if (packagesResult.status === 'fulfilled') {
        ...
        setPackageInfo(packagesResult.value.find((item) => item.id === packageId) || null)
      }
      setLoading(false)
    })
    return () => {
  ...
  const selectedSong = selectedCard?.songs[songIndex] || null
  ...
  return (
    <div className="package-preview-page">
      <section className="panel package-preview-library-switcher" aria-label="切换预览曲库">
        <div className="stack package-preview-library-copy">
          <span className="eyebrow">LIBRARY SELECT</span>
          <strong>选择曲库</strong>
          <span className="muted small">无需返回首页，直接切换要浏览和试听的服务器曲库。</span>
        </div>
        <div className="field package-preview-library-field">
          <label htmlFor="packagePreviewLibrary">当前曲库</label>
          <select
      ...
      <section className="hero package-preview-hero">
        <div>
          ...
          <h1>{catalog.deckName}</h1>
          <p>先浏览曲库和卡面，点击曲目即可试听；页面不会下载整个 ZIP。</p>
          <div className="package-preview-stats">
            ...
            <span className="chip">{packageInfo ? formatBytes(packageInfo.size) : '服务器牌组'}</span>
          </div>
        </div>
      ...
      <div className="package-preview-layout">
        <section className="panel stack">
          <div className="row spread">
            <div>
              <strong>曲库卡面</strong>
              <div className="muted small">滚动时只挂载附近卡面，图片按需读取</div>
            </div>
          ...
          {!selectedCard ? (
            ...
            <>
              ...
              <div className="stack package-preview-detail-heading">
                ...
                <span className="muted small">{selectedCard.songCount} 首曲目 · 选择曲目后试听</span>
              </div>
              ...
              <div className="package-preview-audio">
                <div className="row spread">
                  <strong>单曲试听</strong>
                  <span className="muted small">按需请求</span>
                </div>
                <span className="muted small">
                  当前：{selectedSong?.displayName || '请选择曲目'}。只读取这首音频，不读取整个数据包。
                </span>
                {audioSource ? (
                  <audio
                    key={audioSource}
                    ...
                    preload="none"
                    src={audioSource}
                    onError={() => setAudioMessage('音频暂时无法读取，请稍后重试')}
                    onPlay={() => setAudioMessage(null)}
                  />
                ) : null}

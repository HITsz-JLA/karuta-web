...
app.get('/api/packages/:id/audio', async (request, response, next) => {
  try {
    const packagePath = packagePathFromId(request.params.id)
    const cardKey = typeof request.query.cardKey === 'string' ? request.query.cardKey : ''
    const songIndex = Number.parseInt(typeof request.query.songIndex === 'string' ? request.query.songIndex : '', 10)
    if (!packagePath || !cardKey || !Number.isInteger(songIndex) || songIndex < 0) {
      response.status(400).json({ message: '音频预览参数无效' })
      return
    }

    const catalog = await onlineRooms.getPackageCatalog(request.params.id)
    const card = catalog && findCatalogCard(catalog, cardKey)
    const song = card?.songs?.[songIndex]
    if (!song) {
      response.status(404).json({ message: '预览音频不存在' })
      return
    }

    const media = await readZipAssetRange(packagePath, song.sourcePath, song.fileName, request.headers.range)
    const range = media.range
    const totalBytes = media.totalBytes
    response.setHeader('Content-Type', audioMime(media.name || song.fileName))
    ...
    if (error?.code === 'ENOENT') {
      response.status(404).json({ message: '预览音频资源不存在' })
      return
    }
    next(error)
  }

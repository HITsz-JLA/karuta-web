...
export interface ServerPackageCatalogCard {
  ...
  workName: string
  songCount: number
  songs: ServerPackageCatalogSong[]
}

export interface ServerPackageCatalogSong {
  index: number
  displayName: string
}
...
export function serverCardAudioUrl(packageId: string, cardKey: string, songIndex: number): string {
  const query = new URLSearchParams({ cardKey, songIndex: String(songIndex) })
  return `/api/packages/${encodeURIComponent(packageId)}/audio?${query}`
}
